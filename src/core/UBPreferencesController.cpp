...
// #316: build commit hash, generated by CI (tag or commit). Lets a non-release
// build (version "0.0.0-dev") be identified from the Preferences dialog.
...
void UBPreferencesController::wire()
{
    ...
    // #316: show the clean version plus the build commit hash, so a dev/test
    // build ("0.0.0-dev") is identifiable. e.g. "0.0.0-dev (build 8bab2175)".
    const QString versionWithBuild =
        UBApplication::applicationVersion()
        + QStringLiteral(" (build ") + QStringLiteral(GIT_COMMIT) + QStringLiteral(")");
    mPreferencesUI->versionLabel->setText(tr("version: ") + versionWithBuild);
    ...
    connect(mPreferencesUI->copyVersionButton, &QPushButton::clicked, this, [this]() {
        // #316: copy version + build hash so bug reports carry the exact build.
