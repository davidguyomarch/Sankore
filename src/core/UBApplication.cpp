...
// #309: append a timestamped shutdown-timing line to startup.log so we can
// tell, from a VM run, whether quit is slow in closing()/cleanup() or whether
// the watchdog fires. Lightweight, append-only.
static void ubLogQuitTiming(const QString& line)
{
...
void UBApplication::closing()
{
    ...
    ubLogQuitTiming(QString("after boardController->closing t=%1ms").arg(quitTimer.elapsed()));
...
{
    // #309: reaching cleanup() means the event loop unwound cleanly (the
    // watchdog was not needed). Timestamp the teardown to spot slow deletes.
    QElapsedTimer cleanupTimer;
    cleanupTimer.start();
    ubLogQuitTiming("cleanup() BEGIN (event loop unwound, watchdog not needed)");

    if (applicationController) delete applicationController;
    if (boardController) delete boardController;
    ubLogQuitTiming(QString("after delete board/appController t=%1ms").arg(cleanupTimer.elapsed()));
    if (webController) delete webController;
    if (documentController) delete documentController;
    ubLogQuitTiming(QString("cleanup() END t=%1ms").arg(cleanupTimer.elapsed()));
