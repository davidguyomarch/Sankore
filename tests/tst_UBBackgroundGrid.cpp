...
void TestUBBackgroundGrid::testSeyesHasVerticalAndMargin()
{
    ...
    // default pageLeft==0 the margin is 40 mm from x==0. (16 mm cell spacing
    // doubled by #379 is exercised by testSeyesInterlineSpacing.)
...
void TestUBBackgroundGrid::testGridVerticalsAnchoredToPageLeft_regression362()
{
    ...
    // = -640 (a multiple of the doubled 64 u step) a line does land on 0, so
    // instead assert the phase: every vertical is an integer number of steps
    // from pageLeft. (#379 doubled the Grid step to 16 mm == 64 u.)
    const double step = 16.0 * UPM; // 64
    for (const auto& l : lines)
    {
