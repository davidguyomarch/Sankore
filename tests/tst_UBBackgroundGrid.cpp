...
void TestUBBackgroundGrid::testSeyesHasVerticalAndMargin()
{
    ...
    // default pageLeft==0 the margin is 40 mm from x==0. (#379 doubled the cell
    // spacing but does not change this test's assertions.)
...
void TestUBBackgroundGrid::testGridVerticalsAnchoredToPageLeft_regression362()
{
    ...
    // Grid verticals are phased from the page left edge; assert the phase:
    // every vertical is an integer number of (doubled, #379: 16 mm == 64 u)
    // steps from pageLeft.
    const double step = 16.0 * UPM; // 64 (#379)
    for (const auto& l : lines)
    {
